...
/// Links a guest account to Apple (iOS only) and persists the named user profile.
class LinkWithAppleUseCase {
  const LinkWithAppleUseCase({
  ...
  Future<({UserProfile? profile, Failure? failure})> execute({
    ...
    final result = await _auth.linkWithApple(guestUid: guestUid);
